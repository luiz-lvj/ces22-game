...
class LevelOne:
        ...
        for i in range(4):
            for j in range(4):
                pygame.draw.rect(self.screen, dictcolor1[TABLE[i][j]], (j*boxsize+margin,
                                                ...
                                                thickness)
                if self.table[i][j] != 0:
                    order=int(math.log10(self.table[i][j]))
                    myfont = pygame.font.SysFont("Arial", fontSize[order] , bold=True)
                    label = myfont.render("%4s" %(self.table[i][j]), 1, dictcolor2[self.table[i][j]] )
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if running:
                        ...
                        new_table = self.key(desired_key, copy.deepcopy(self.table))
    ...
    def key(self,direction, T):
        if   direction =='w':
            for pi in range(1,4):
                for pj in range(4):
                    if T[pi][pj] !=0: T=self.moveup(pi,pj,T)
        elif direction =='s':
            for pi in range(2,-1,-1):
                for pj in range(4):
                    if T[pi][pj] !=0: T=self.movedown(pi,pj,T)
        elif direction =='a':
            for pj in range(1,4):
                for pi in range(4):
                    if T[pi][pj] !=0: T=self.moveleft(pi,pj,T)
        elif direction =='d':
            for pj in range(2,-1,-1):
                for pi in range(4):
                    if T[pi][pj] !=0: T=self.moveright(pi,pj,T)
        return T
